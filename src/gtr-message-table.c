...
static void
gtr_message_table_selection_changed (GObject *object, GParamSpec *pspec,
...
gtr_message_table_init (GtrMessageTable *table)
{
  ...
  priv->id_sorter = GTK_SORTER (gtk_numeric_sorter_new (
  ...
  g_settings_bind (priv->ui_settings, GTR_SETTINGS_SHOW_ID_COLUMN,
                   priv->idlabel, "visible", G_SETTINGS_BIND_DEFAULT);
}
                                ...
                                GParamSpec   *pspec)
{
  ...
  switch (prop_id)
    ...
    case PROP_TAB:
      ...
      g_signal_connect (priv->tab, "showed-message",
...
gtr_message_table_class_init (GtrMessageTableClass *klass)
{
  ...
  gtk_widget_class_set_template_from_resource (
      widget_class, "/org/gnome/Gtranslator/ui/gtr-message-table.ui");
                            ...
                            GtrMessageContainer *container)
{
  ...
  gtk_list_view_set_model (GTK_LIST_VIEW (priv->messages),
                           GTK_SELECTION_MODEL (priv->selection));
  g_signal_connect (priv->selection, "notify::selected",
                            ...
                            GtrMessageTableNavigationFunc func)
{
  ...
  switch (navigation)
    ...
    case GTR_NAVIGATE_FIRST:
      gtk_single_selection_set_selected (
          GTK_SINGLE_SELECTION (priv->selection), 0);
      break;
    case GTR_NAVIGATE_LAST:
      {
        int n_children
            = g_list_model_get_n_items (G_LIST_MODEL (priv->selection));
        gtk_single_selection_set_selected (
            GTK_SINGLE_SELECTION (priv->selection), n_children - 1);
      }
      break;
    case GTR_NAVIGATE_NEXT:
      if (func)
        {
          int next = gtk_single_selection_get_selected (
                         GTK_SINGLE_SELECTION (priv->selection))
                     + 1;
          while (TRUE)
            {
              g_autoptr (GtrMsg) msg = NULL;
              msg = GTR_MSG (g_list_model_get_object (
                  G_LIST_MODEL (priv->sort_model), next));
              if (!msg)
                return NULL;
              ...
              if (func (msg))
                {
                  gtk_single_selection_set_selected (
                      GTK_SINGLE_SELECTION (priv->selection), next);
        ...
        {
          int selected = gtk_single_selection_get_selected (
              GTK_SINGLE_SELECTION (priv->selection));
          gtk_single_selection_set_selected (
              GTK_SINGLE_SELECTION (priv->selection), selected + 1);
        }
    ...
    case GTR_NAVIGATE_PREV:
      if (func)
        {
          int prev = gtk_single_selection_get_selected (
                         GTK_SINGLE_SELECTION (priv->selection))
                     - 1;
          while (TRUE)
            {
              g_autoptr (GtrMsg) msg = NULL;
              msg = GTR_MSG (g_list_model_get_object (
                  G_LIST_MODEL (priv->sort_model), prev));
              if (!msg)
                return NULL;
              ...
              if (func (msg))
                {
                  gtk_single_selection_set_selected (
                      GTK_SINGLE_SELECTION (priv->selection), prev);
                  return g_steal_pointer (&msg);
                }
  ...
  g_autoptr (GtrMsg) msg = NULL;
  msg = g_object_ref (gtk_single_selection_get_selected_item (
      GTK_SINGLE_SELECTION (priv->selection)));
  return g_steal_pointer (&msg);
}

void
gtr_message_table_sort_by (GtrMessageTable *table, GtrMessageTableSortBy sort)
{
  GtrMessageTablePrivate *priv;
  priv = gtr_message_table_get_instance_private (table);
  priv->sort_status = sort;

  if (!priv->store || priv->sort_model == NULL
      || !GTK_IS_SORT_LIST_MODEL (priv->sort_model))
    return;
  ...
  switch (sort)
    ...
    case GTR_MESSAGE_TABLE_SORT_TRANSLATED:
      gtk_sort_list_model_set_sorter (priv->sort_model,
                                      priv->translation_sorter);
      break;
...
gtr_message_table_select (GtrMessageTable *table, GtrMsg *msg)
{
  ...
  gtk_single_selection_set_selected (GTK_SINGLE_SELECTION (priv->selection),
                                     position);
}
