...
void gtranslator_parse_main(const gchar *filename)
{
	...
	{
		autosave_source_tag=g_timeout_add((GtrPreferences.autosave_timeout * 60000),
...
gboolean gtranslator_save_file(const gchar *name)
{
	 ...
	 *  also not disable the file saving entries.
	 */
	if(!GtrPreferences.autosave_with_suffix && (autosave_source_tag > 1))
	{
