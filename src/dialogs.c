...
static void goto_dlg_clicked(GnomeDialog *dialog,gint button,gpointer adjustment);
static void match_case_toggled(GtkWidget *widget,gpointer useless);
static void find_dlg_clicked(GnomeDialog *dialog,gint button,gpointer findy);
...
//	gtk_file_selection_complete(GTK_FILE_SELECTION(dialog),"*.po*");
	...
	gtk_signal_connect(GTK_OBJECT(GTK_FILE_SELECTION(dialog)->ok_button),
		"clicked",
		GTK_SIGNAL_FUNC(parse_the_file),(gpointer)dialog);
...
{
	if (button==GNOME_OK)
		goto_nth_msg(GTK_WIDGET(dialog),adjustment);
	gnome_dialog_close(dialog);
}

void goto_dlg(GtkWidget *widget,gpointer useless)
{
	static GtkWidget *g_dlg=NULL;
	static GtkObject *adjustment;
	GtkWidget *g_dlg_line,*g_dlg_label;

	if (g_dlg) 
		gtk_adjustment_set_value(GTK_ADJUSTMENT(adjustment),
					 g_list_position(messages,cur_msg)+1);
	raise_and_return_if_exists(g_dlg);
	g_dlg=gnome_dialog_new(_("GoTo message entry"),
	                       _("Go!"),
			       GNOME_STOCK_BUTTON_CANCEL,
			       NULL);
	// We want Go! button to be the default
	gnome_dialog_set_default(GNOME_DIALOG(g_dlg),0);
	...
	adjustment=gtk_adjustment_new(g_list_position(messages,cur_msg)+1,
	                              1, g_list_length(messages), 1, 10, 10);
	g_dlg_line=gtk_spin_button_new(GTK_ADJUSTMENT(adjustment),1,0);
	gnome_dialog_editable_enters(GNOME_DIALOG(g_dlg),
	                             GTK_EDITABLE(g_dlg_line));
	gtk_window_set_focus(GTK_WINDOW(g_dlg),g_dlg_line);
	/**
	* Add'em to the dialog
	**/
	gtk_box_pack_start(GTK_BOX(GNOME_DIALOG(g_dlg)->vbox),
			g_dlg_label,
			FALSE,FALSE,0);
	gtk_box_pack_start(GTK_BOX(GNOME_DIALOG(g_dlg)->vbox),
			g_dlg_line,
	...
	* Connect the signals
	**/
	gtk_signal_connect(GTK_OBJECT(g_dlg), "clicked",
        		GTK_SIGNAL_FUNC(goto_dlg_clicked),
			adjustment);
	show_nice_dialog(&g_dlg,"gtranslator -- goto");
}
static void match_case_toggled(GtkWidget *widget,gpointer useless)
{
	if_match_case=!if_match_case;
	gnome_config_set_bool("/gtranslator/Toggles/Case sensitive",if_match_case);
	gnome_config_sync();
}
...
static void find_dlg_clicked(GnomeDialog *dialog,gint button,gpointer findy)
{
	...
	if (button==GNOME_OK)
	{
		search_for=gtk_editable_get_chars(GTK_EDITABLE(gnome_entry_gtk_entry(findy)),0,-1);
		search_do(NULL,search_for);
		enable_actions("find_again");
		return;
	}
	gnome_dialog_close(dialog);
}
...
void find_dialog(GtkWidget *widget,gpointer useless)
{
	...
	raise_and_return_if_exists(dialog);
	dialog=gnome_dialog_new(_("Find in the po-file"),
	                       _("Find"),
	...
	**/
	...
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(match_case),if_match_case);
	/**
	...
	**/
	...
	gtk_box_pack_start(GTK_BOX(GNOME_DIALOG(dialog)->vbox),
			findy,FALSE,FALSE,0);
	gtk_box_pack_start(GTK_BOX(GNOME_DIALOG(dialog)->vbox),
			match_case,FALSE,FALSE,0);
	/**
	* Connect the signals
	**/
	gtk_signal_connect(GTK_OBJECT(dialog), "clicked",
        		GTK_SIGNAL_FUNC(find_dlg_clicked),
			findy);
	gtk_signal_connect(GTK_OBJECT(match_case), "toggled",
        		GTK_SIGNAL_FUNC(match_case_toggled),
			NULL);
...
void query_dialog(GtkWidget *widget,gpointer useless)
{
	...
	**/
	...
	gtk_window_set_wmclass(GTK_WINDOW(q_dlg),"gtranslator -- query","gtranslator");
...
void r_window(GtkWidget *widget,gpointer useless)
{
	...
	**/
	...
	gtk_window_set_wmclass(GTK_WINDOW(r_window_dlg),"gtranslator -- result","gtranslator");	
	/**
